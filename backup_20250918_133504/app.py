...
from rss_scheduler import start_rss_scheduler, get_scheduler_status
import json
import re
...
# Initialize sync service and RSS manager
...
@login_required
def get_rss_feeds():
    """Get all RSS feeds with status information"""
    try:
...
def add_rss_feed():
    ...
    try:
        ...
        success, message = rss_manager.add_feed(name, url, description, category, update_interval)
...
def update_rss_feed(feed_id):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE rss_feeds 
            SET name = ?, description = ?, category = ?, update_interval = ?, is_active = ?
            WHERE id = ?
        ''', (
            data.get('name'),
            data.get('description'),
            data.get('category'),
            data.get('update_interval'),
...
def delete_rss_feed(feed_id):
    ...
    try:
        ...
        # Delete feed (cascade will handle related records)
...
def refresh_rss_feed(feed_id):
    ...
    try:
        results = rss_manager.process_feed(feed_id)
        return jsonify({
...
def refresh_all_rss_feeds():
    """Manually refresh all active RSS feeds"""
    try:
...
@app.route('/api/rss-feeds/logs')
@login_required
def get_rss_feed_logs():
    """Get RSS feed activity logs"""
    try:
        feed_id = request.args.get('feed_id', type=int)
        limit = request.args.get('limit', 50, type=int)
        
        logs = rss_manager.get_feed_logs(feed_id, limit)
        return jsonify({'logs': logs})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/rss-feeds/categories')
@login_required
def get_feed_categories():
    ...
    try:
        ...
        cursor.execute('SELECT * FROM feed_categories ORDER BY name')
        categories = [dict(row) for row in cursor.fetchall()]
...
@login_required
def override_event_field(event_id):
    """Override a specific field of an event (manual override)"""
    try:
        data = request.get_json()
        ...
        if not field_name or override_value is None:
            return jsonify({'error': 'Field name and override value are required'}), 400
        
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        
        # Get current value
        cursor.execute(f'SELECT {field_name} FROM events WHERE id = ?', (event_id,))
        ...
        # Add override record
        cursor.execute('''
            INSERT INTO manual_overrides (event_id, field_name, original_value, override_value, override_reason)
            VALUES (?, ?, ?, ?, ?)
        ''', (event_id, field_name, current_value[0], override_value, override_reason))
        
        # Update the event
        cursor.execute(f'UPDATE events SET {field_name} = ? WHERE id = ?', (override_value, event_id))
        
        conn.commit()
        conn.close()
        
        return jsonify({'message': 'Event field overridden successfully'})
        
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/rss-feeds/scheduler-status')
@login_required
...
if __name__ == '__main__':
    ...
    # Start RSS scheduler automatically
